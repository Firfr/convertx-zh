...
#     # 使用 perl 统计替换次数并输出到标准错误
#     local count=$(perl -i -CSD -Mutf8 -pe \
#         "BEGIN{\$orig=quotemeta('$original'); \
#               \$repl='$replacement'} \
#          \$count += s/\$orig/\$repl/g; \
#          END { print STDERR \$count }" "$target_file" 2>&1)
...
perform_replacement() {
    ...
    # 使用 awk 命令，并将结果（计数值）重定向到一个临时变量文件
    # 这样可以避免复杂的命令替换和重定向冲突
    awk -v orig="$original" -v repl="$replacement" '
        BEGIN { cnt = 0 }
        {
            cnt += gsub(orig, repl)
            print
        }
        END { print cnt > "/dev/stderr" }
    ' "$target_file" > "$temp_file" 2> "${temp_file}.count"

    # 检查 awk 命令是否成功执行
    if [[ $? -eq 0 ]]; then
        # 从临时文件中读取计数值
        count=$(cat "${temp_file}.count")
        # 用修改后的临时文件覆盖原文件
        mv "$temp_file" "$target_file"
    else
        count=0 # 如果 awk 失败，计数为 0
    fi

    # 清理临时文件
    rm -f "$temp_file" "${temp_file}.count"
