...
export const history = new Elysia()
  ...
  .get("/history", async ({ jwt, redirect, cookie: { auth } }) => {
    ...
    return (
      <BaseHtml webroot={WEBROOT} title="ConvertX | 结果">
        <>
          <Header
            webroot={WEBROOT}
                  ...
                  {userJobs.map((job) => (
                    <>
                      <tr id={`job-row-${job.id}`}>
                        ...
                        <td safe>{job.status  === 'completed' ? '完成' : job.status}</td>
                        <td>
                          <a
                            class={`
                      ...
                      <tr id={`details-${job.id}`} class="hidden">
                        <td colspan="6">
                          <div class="p-2 text-sm text-neutral-500">
                            <div class="mb-1 font-semibold">详细文件信息:</div>
